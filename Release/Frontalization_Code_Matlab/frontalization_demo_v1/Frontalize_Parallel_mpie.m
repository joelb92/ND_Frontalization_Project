...
%
% clear;
function Frontalize_Parallel(image_dir,image_type,output_dir,detector,frontalization,numCores,faceSize,progressDir,jobName)
% close all;
% clear all;
...
parfor i = 1:nfiles
    ...
    [full_dir,fname,ext] = fileparts(filename);
            ...
            [save_dir_failed_dir,failedname,faildext] = fileparts(save_dir_failed);
            mkdir(save_dir_failed_dir);
    ...
    try
        ...
        try
            copyfile(filename,fullfile(save_dir_failed_dir,strcat(fname,ext)),'f')
        catch
        end
        continue;
    end
