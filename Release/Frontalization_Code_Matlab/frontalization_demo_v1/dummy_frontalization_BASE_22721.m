...
%% Init stuff
function [warped_surface, landmarked_img]=dummy_frontalization(test_pic)
% load model
%clear all
...
mpath = '/Users/abharati/Desktop/HassnerS-Frontalization/frontalize.0.1.2/Vito/model/';
...
% maped1 = [mapped_img(:,1:75,:), mapped_img1(:,76:end,:)];
% imshow(uint8(maped1),[])
% subplot(2,4,4)
% title('3D Warpped - hard symmetric 1')
% maped2 = [mapped_img1(:,1:75,:), mapped_img(:,76:end,:)];
% imshow(uint8(maped2),[])
% 
% subplot(2,4,5)
% title('3D Warpped - hard symmetric 1')
% maped1 = [mapped_img(:,1:76,:), mapped_img1(:,74:end,:)];
% imshow(uint8(maped1),[])
% subplot(2,4,6)
% title('3D Warpped - hard symmetric 1')
% maped2 = [mapped_img1(:,1:76,:), mapped_img(:,76:end,:)];
% imshow(uint8(maped2),[])
% 
% subplot(2,4,7)
% title('3D Warpped - hard symmetric 1')
% maped1 = [mapped_img(:,1:77,:), mapped_img1(:,73:end,:)];
% imshow(uint8(maped1),[])
% subplot(2,4,8)
% title('3D Warpped - hard symmetric 1')
% maped2 = [mapped_img1(:,1:77,:), mapped_img(:,73:end,:)];
% imshow(uint8(maped2),[])
