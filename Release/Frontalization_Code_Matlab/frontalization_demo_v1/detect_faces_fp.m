...
%                                 'MinNeighbors', 1, ...
%                                 'MinSize',      [20, 20]); 
    boxestmp = step(VJdetector.fdetector, X2);                         
    % convert boxes to cell array
    boxes={};
    for jj=1:size(boxestmp,1)
        boxes{1,jj}=boxestmp(jj,:);
    end
...
%                                     'MinSize',      [20, 20]);
      ...
      boxes1tmp = step(VJdetector.pdetector, X2);  

      boxes1={};
      for jj=1:size(boxes1tmp,1)
          boxes1{1,jj}=boxes1tmp(jj,:);
      end
      bbox1=[];
      if numel(boxes1)>=1 
