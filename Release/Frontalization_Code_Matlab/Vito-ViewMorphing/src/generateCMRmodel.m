...
matches = strfind(path_list_cell,'mexopencv');
if any(vertcat(matches{:}))
    VJdetector.down_sampling_size = 320; 
    VJdetector.fdetector = cv.CascadeClassifier('haarcascade_frontalface_alt2.xml');
    VJdetector.pdetector = cv.CascadeClassifier('haarcascade_profileface.xml');
    amodel.VJdetector = VJdetector;
else
    disp('mexopencv is not in Matlab path. Add path or use some other face detector.');
end
...
load('model_frontal.mat')
amodel.fd_frontal = npdModel;
load('model_unconstrain.mat')
amodel.fd_unconstrain = npdModel;
